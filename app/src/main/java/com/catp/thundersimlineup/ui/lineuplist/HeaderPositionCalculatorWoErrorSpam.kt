...
class HeaderPositionCalculatorWoErrorSpam(
     ...
     */
    private val mTempRect1: Rect = Rect()
    private val mTempRect2: Rect = Rect()
    ...
    private fun initDefaultHeaderOffset(
        ...
        val layoutParams: ViewGroup.LayoutParams = firstView.getLayoutParams()
        var leftMargin = 0
        var topMargin = 0
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            translationX = firstView.getX().toInt() - leftMargin + mTempRect1.left
            translationY = Math.max(
                firstView.getY().toInt() - topMargin - header.getHeight() - mTempRect1.bottom,
                getListTop(recyclerView) + mTempRect1.top
            )
        } else {
            translationY = firstView.getY().toInt() - topMargin + mTempRect1.top
            translationX = Math.max(
                firstView.getX().toInt() - leftMargin - header.getWidth() - mTempRect1.right,
                getListLeft(recyclerView) + mTempRect1.left
            )
        }
        headerMargins.set(
            translationX, translationY, translationX + header.getWidth(),
    ...
    private fun isStickyHeaderBeingPushedOffscreen(
        ...
        if (viewAfterHeader != null) {
            val firstViewUnderHeaderPosition =
                recyclerView.getChildAdapterPosition(viewAfterHeader)
            ...
            if (firstViewUnderHeaderPosition > 0 && hasNewHeader(
                ...
                if (mOrientationProvider.getOrientation(recyclerView) == LinearLayoutManager.VERTICAL) {
                    val topOfNextHeader: Int =
                        viewAfterHeader.getTop() - mTempRect1.bottom - nextHeader.getHeight() - mTempRect1.top
                    val bottomOfThisHeader: Int =
                        recyclerView.paddingTop + stickyHeader.getBottom() + mTempRect2.top + mTempRect2.bottom
                    if (topOfNextHeader < bottomOfThisHeader) {
                        return true
                    }
                } else {
                    val leftOfNextHeader: Int =
                        viewAfterHeader.getLeft() - mTempRect1.right - nextHeader.getWidth() - mTempRect1.left
                    val rightOfThisHeader: Int =
                        recyclerView.paddingLeft + stickyHeader.getRight() + mTempRect2.left + mTempRect2.right
    ...
    private fun translateHeaderWithNextHeader(
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            val topOfStickyHeader: Int =
                getListTop(recyclerView) + mTempRect2.top + mTempRect2.bottom
            val shiftFromNextHeader: Int =
                viewAfterNextHeader.getTop() - nextHeader.getHeight() - mTempRect1.bottom - mTempRect1.top - currentHeader.getHeight() - topOfStickyHeader
            if (shiftFromNextHeader < topOfStickyHeader) {
                translation.top += shiftFromNextHeader
            }
        } else {
            val leftOfStickyHeader: Int =
                getListLeft(recyclerView) + mTempRect2.left + mTempRect2.right
            val shiftFromNextHeader: Int =
                viewAfterNextHeader.getLeft() - nextHeader.getWidth() - mTempRect1.right - mTempRect1.left - currentHeader.getWidth() - leftOfStickyHeader
    ...
    private fun itemIsObscuredByHeader(
        ...
        val layoutParams =
            item.getLayoutParams() as RecyclerView.LayoutParams
        mDimensionCalculator.initMargins(mTempRect1, header)
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            val itemTop: Int = item.getTop() - layoutParams.topMargin
            ...
            val itemLeft: Int = item.getLeft() - layoutParams.leftMargin
            val headerRight: Int =
