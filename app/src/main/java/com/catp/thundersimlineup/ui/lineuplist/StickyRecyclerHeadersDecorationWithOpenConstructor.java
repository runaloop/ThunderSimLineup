...
public class StickyRecyclerHeadersDecorationWithOpenConstructor extends RecyclerView.ItemDecoration {
    ...
    private final HeaderPositionCalculatorWoErrorSpam mHeaderPositionCalculatorWoErrorSpam;
    ...
    public StickyRecyclerHeadersDecorationWithOpenConstructor(StickyRecyclerHeadersAdapter adapter) {
        this(adapter, new LinearLayoutOrientationProvider(), new DimensionCalculator(), null);
    }

    public StickyRecyclerHeadersDecorationWithOpenConstructor(StickyRecyclerHeadersAdapter adapter, ItemVisibilityAdapter visibilityAdapter) {
        this(adapter, new LinearLayoutOrientationProvider(), new DimensionCalculator(), visibilityAdapter);
    }

    private StickyRecyclerHeadersDecorationWithOpenConstructor(StickyRecyclerHeadersAdapter adapter, OrientationProvider orientationProvider,
    ...
    private StickyRecyclerHeadersDecorationWithOpenConstructor(StickyRecyclerHeadersAdapter adapter, OrientationProvider orientationProvider,
        ...
        this(adapter, headerRenderer, orientationProvider, dimensionCalculator, headerProvider,
                new HeaderPositionCalculatorWoErrorSpam(adapter, headerProvider, orientationProvider,
                        dimensionCalculator), visibilityAdapter);
    }

    public StickyRecyclerHeadersDecorationWithOpenConstructor(StickyRecyclerHeadersAdapter adapter, HeaderRenderer headerRenderer,
                                                              OrientationProvider orientationProvider, DimensionCalculator dimensionCalculator, HeaderProvider headerProvider,
                                                              HeaderPositionCalculatorWoErrorSpam HeaderPositionCalculatorWoErrorSpam, ItemVisibilityAdapter visibilityAdapter) {
        mAdapter = adapter;
        mHeaderProvider = headerProvider;
        mOrientationProvider = orientationProvider;
        mRenderer = headerRenderer;
        mDimensionCalculator = dimensionCalculator;
        mHeaderPositionCalculatorWoErrorSpam = HeaderPositionCalculatorWoErrorSpam;
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mHeaderPositionCalculatorWoErrorSpam.hasNewHeader(itemPosition, mOrientationProvider.isReverseLayout(parent))) {
    ...
    public void onDrawOver(Canvas canvas, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            boolean hasStickyHeader = mHeaderPositionCalculatorWoErrorSpam.hasStickyHeader(itemView, mOrientationProvider.getOrientation(parent), position);
            if (hasStickyHeader || mHeaderPositionCalculatorWoErrorSpam.hasNewHeader(position, mOrientationProvider.isReverseLayout(parent))) {
                ...
                mHeaderPositionCalculatorWoErrorSpam.initHeaderBounds(headerOffset, parent, header, itemView, hasStickyHeader);
